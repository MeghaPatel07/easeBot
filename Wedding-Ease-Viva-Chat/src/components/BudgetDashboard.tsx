...
export default function BudgetDashboard({ userId }: BudgetDashboardProps) {
  ...
  if (budget === null) {
    return (
      <div className="flex flex-col items-center justify-center h-full px-6">
        <div className="w-full max-w-sm bg-white/70 backdrop-blur-sm rounded-2xl border border-border shadow-sm p-8 text-center">
          <div className="h-12 w-12 rounded-2xl bg-blue-50 flex items-center justify-center mx-auto mb-4">
            <DollarSign className="h-6 w-6 text-primary" />
          </div>
          <h2 className="text-base font-semibold text-stone-800 mb-1">Set Your Wedding Budget</h2>
          <p className="text-xs text-stone-400 mb-6">Enter your total budget to get started tracking expenses.</p>
          <input
            type="number"
            min="0"
            value={setupAmount}
            ...
            className="w-full text-sm bg-white border border-primary/20 rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-primary/20 placeholder:text-stone-300 mb-4 text-center"
          />
          <button
            onClick={() => {
  ...
  return (
    <div className="flex flex-col h-full bg-white/60 backdrop-blur-sm rounded-2xl border border-border shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-semibold text-stone-800">Budget Tracker</h2>
            <p className="text-xs text-stone-400">{budget.categories.length} categories</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 py-4 border-b border-border flex-shrink-0">
        <div className="flex items-center justify-between mb-2">
          <div className="flex gap-4">
            <div>
              <span className="text-2xs text-stone-400 font-medium block">Total Budget</span>
              <span className="text-sm font-semibold text-stone-800">{fmt.format(budget.totalBudget)}</span>
            </div>
            <div>
              <span className="text-2xs text-stone-400 font-medium block">Spent</span>
              <span className={`text-sm font-semibold ${statusColors[overallStatus].text}`}>{fmt.format(totalSpent)}</span>
            </div>
            <div>
              <span className="text-2xs text-stone-400 font-medium block">Remaining</span>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <div key={cat.id} className={`rounded-2xl border ${colors.border} ${colors.bg} overflow-hidden transition-all duration-200`}>
              ...
              >
                ...
                {isEditing ? (
                  <div className="flex-1 flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                    ...
                    <button onClick={() => setEditingCatId(null)} className="h-6 w-6 flex items-center justify-center rounded-lg text-stone-400 hover:bg-stone-100 transition-colors">
                      <X className="h-3 w-3" />
                    </button>
                  </div>
                ) : (
                  <>
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                      <button
                        ...
                        className="h-6 w-6 flex items-center justify-center rounded-lg text-stone-300 hover:text-primary hover:bg-white/80 transition-colors"
                        title="Edit category"
                      >
                      ...
                      <button
                        ...
                        className="h-6 w-6 flex items-center justify-center rounded-lg text-stone-300 hover:text-red-400 hover:bg-white/80 transition-colors"
              ...
              {isExpanded && !isEditing && (
                <div className="px-4 pb-3 space-y-1.5">
                  {cat.items.length === 0 && (
                    <p className="text-xs text-stone-400 py-2 text-center">No expenses yet.</p>
                  )}
                  {cat.items.map((item) => (
                    <div
                      key={item.id}
                      className="flex items-center gap-2 p-2 rounded-xl bg-white/70 border border-border"
                    >
                      <button
                      ...
                      <div className="flex-1 min-w-0">
                        <span className={`text-sm block truncate ${item.paid ? 'line-through text-stone-400' : 'text-stone-700'}`}>
                          {item.description}
                        </span>
                      ...
                      <span className="text-xs font-semibold text-stone-600 flex-shrink-0">{fmt.format(item.amount)}</span>
                      <button
                        onClick={() => deleteLineItem(userId, cat.id, item.id)}
                        className="h-6 w-6 flex items-center justify-center rounded-lg text-stone-300 hover:text-red-400 hover:bg-red-50 transition-colors flex-shrink-0"
                        title="Delete item"
                      >
                        <Trash2 className="h-3 w-3" />
                      </button>
                  ...
                  {/* Add line item form */}
                  {showAddItem === cat.id ? (
                    <div className="flex flex-col gap-2 p-2 rounded-xl bg-white/70 border border-border">
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="flex-1 text-sm bg-white border border-primary/20 rounded-lg px-2 py-1 outline-none focus:ring-1 focus:ring-primary/20 placeholder:text-stone-300"
                        />
                        <input
                          type="number"
                          min="0"
                          ...
                          placeholder="Amount"
                          className="w-20 text-sm bg-white border border-primary/20 rounded-lg px-2 py-1 outline-none focus:ring-1 focus:ring-primary/20 placeholder:text-stone-300"
                        />
                      </div>
                      <div className="flex items-center gap-2">
                        <input
        ...
        {/* Add category form */}
        <div className="rounded-2xl border border-dashed border-primary/20 bg-white/40 p-4">
          <p className="text-xs text-stone-400 font-medium mb-2">Add Category</p>
          <div className="flex items-center gap-2">
            <input
              value={newCatName}
              ...
              className="flex-1 text-sm bg-white border border-primary/20 rounded-xl px-3 py-2 outline-none focus:ring-2 focus:ring-primary/20 placeholder:text-stone-300"
            />
            <input
              type="number"
              min="0"
              ...
              onKeyDown={(e) => { if (e.key === 'Enter') handleAddCategory() }}
              placeholder="Budget"
              className="w-24 text-sm bg-white border border-primary/20 rounded-xl px-3 py-2 outline-none focus:ring-2 focus:ring-primary/20 placeholder:text-stone-300"
            />
            <button
