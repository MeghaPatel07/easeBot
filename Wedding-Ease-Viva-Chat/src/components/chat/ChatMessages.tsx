        ...
        >
          {message.sender === 'user' ? (
            ...
            <div className="max-w-[calc(100%-2rem)] sm:max-w-sm md:max-w-lg lg:max-w-xl group msg-enter">
              ...
              {/* Follow-up suggestion capsules — generated by the backend per
                  reply and held in memory only (never stored). Shown only under
                ...
                <div className="not-prose mt-3 flex flex-wrap gap-2 animate-in fade-in slide-in-from-bottom-1 duration-300">
                  {message.suggestions.map((q, i) => (
                    <button
                      ...
                      onClick={() => {
                        onSetInputText(q);
                        track('followup_suggestion_clicked', { index: i, mode: message.mode ?? 'auto' });
                      }}
                      className="group/sg inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-primary/25 bg-primary/[0.06] text-xs text-foreground/75 text-left hover:bg-primary/10 hover:border-primary/45 hover:text-primary active:scale-95 transition-all"
                    >
                      <Sparkles className="h-3 w-3 text-primary/60 flex-shrink-0 group-hover/sg:text-primary" />
                      <span>{q}</span>
                    </button>
                  ))}
                </div>
