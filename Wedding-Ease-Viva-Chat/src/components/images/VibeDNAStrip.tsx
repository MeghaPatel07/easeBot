...
export function VibeDNAStrip({ vibe, onAddDescriptor, onRemoveDescriptor, onChangeVibe }: VibeDNAStripProps) {
  ...
  return (
    ...
    >
      ...
      {vibe.subtitle && (
        <p className="mt-1 text-sm text-muted-foreground">{vibe.subtitle}</p>
      )}
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        {vibe.descriptors.map((d) => (
          <span
            key={d}
            className="inline-flex items-center gap-1 rounded-full bg-muted px-3 py-1 text-xs font-medium text-foreground sm:text-sm"
          >
            {d}
            <button
              type="button"
        ...
        {adding ? (
          ...
          <button
            ...
            className="inline-flex items-center gap-1 rounded-full border border-dashed border-border px-3 py-1 text-xs font-medium text-muted-foreground hover:bg-muted hover:text-foreground focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 sm:text-sm"
          >
