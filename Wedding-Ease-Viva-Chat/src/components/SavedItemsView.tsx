    ...
    <div className="flex flex-col h-full">
            ...
            <button
              ...
              className={`px-2.5 py-1 h-9 sm:h-8 rounded-full text-xs font-medium transition-colors ${
                active
                  ...
                  : 'bg-white/70 text-stone-500 border border-stone-200 hover:border-blue-200 hover:text-blue-600'
              }`}
            >
              {cat}
            </button>
      ...
      <div className="flex items-center gap-2 mb-3 flex-shrink-0">
        <div className="flex-1 relative">
          <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-stone-300" />
          <input
            value={searchQuery}
            onChange={e => setSearchQuery(e.target.value)}
            placeholder="Search saved items..."
            className="w-full text-sm bg-white/70 border border-stone-200 rounded-xl pl-8 pr-3 py-2 outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-300 placeholder:text-stone-300"
          />
          {searchQuery && (
            <button
      ...
      {showAddForm && (
        <div className="mb-3 flex-shrink-0 bg-white/70 border border-blue-200 rounded-2xl p-4 space-y-3">
          <textarea
            ...
            rows={3}
            className="w-full text-sm bg-white border border-stone-200 rounded-xl px-3 py-2 outline-none focus:ring-2 focus:ring-blue-200 resize-none placeholder:text-stone-300"
          />
          <div className="flex items-center gap-2">
            <Tag className="h-3.5 w-3.5 text-stone-400 flex-shrink-0" />
          ...
          <input
            ...
            className="w-full text-sm bg-white border border-stone-200 rounded-xl px-3 py-2 outline-none focus:ring-2 focus:ring-blue-200 placeholder:text-stone-300"
          />
      ...
      <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-stone-400">
            ...
            <p className="text-xs text-stone-300 mt-1 text-center max-w-[220px]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            {filtered.map(item => {
              ...
              if (isProduct && productData) {
                return (
                  <a
                    ...
                    className="group relative flex flex-col bg-white/70 border border-border hover:border-blue-200 rounded-2xl overflow-hidden transition-all hover:shadow-md"
                  >
                    <div className="relative overflow-hidden bg-stone-200 flex-shrink-0 h-32 sm:h-40">
                      <img
                      ...
                      <button
                        ...
                        className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 h-7 w-7 flex items-center justify-center rounded-lg bg-white/90 text-stone-400 hover:text-red-500 transition-all"
                        title="Delete"
                      >
                    ...
                    <div className="p-3 flex flex-col flex-1">
                      ...
                      <p className="text-sm font-semibold text-stone-800 line-clamp-2 flex-1">{item.text}</p>
                      {item.sourceThreadTitle && (
                        <p className="text-2xs text-stone-400 mt-auto pt-1.5">{item.sourceThreadTitle}</p>
                      )}
                    </div>
                  </a>
                );
              ...
              return (
                <div
                  key={item.id}
                  className="group relative bg-white/70 border border-border hover:border-border rounded-2xl p-4 transition-all hover:shadow-sm"
                >
                  {/* Delete button */}
                  <button
                    onClick={() => handleDelete(item.id)}
                    className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 h-6 w-6 flex items-center justify-center rounded-lg text-stone-300 hover:text-red-400 hover:bg-red-50 transition-all"
                    title="Delete"
                  >
                            ...
                            <button
                              ...
                              className={`text-3xs px-1.5 py-0.5 rounded-full font-medium transition-all ${
                                item.category === cat
                                  ...
                                  : 'bg-white/60 text-stone-400 border border-stone-200 hover:bg-white'
                              }`}
                            >{cat}</button>
                          )
                        })}
                  ...
                  {/* Text snippet */}
                  <p className="text-sm text-stone-700 leading-snug line-clamp-4 mb-2 pr-6">
                    {item.text}
                  </p>
                  ...
                  {item.sourceThreadTitle && (
                    <div className="flex items-center gap-1 mb-2">
                      <MessageSquare className="h-2.5 w-2.5 text-stone-300 flex-shrink-0" />
                      <span className="text-2xs text-stone-400 truncate">
                        {item.sourceThreadTitle}
                      </span>
                    </div>
                  )}
                  ...
                  {isEditingNote ? (
                    <div className="flex items-center gap-1.5 mb-2">
                      <input
                        ...
                        className="flex-1 text-xs bg-white border border-blue-200 rounded-lg px-2 py-1 outline-none focus:ring-1 focus:ring-blue-300 placeholder:text-stone-300"
                      />
                      <button
                    ...
                    >
                      <Edit3 className="h-2.5 w-2.5 text-stone-300 flex-shrink-0 mt-0.5" />
                      <span className="text-xs text-stone-400 italic leading-snug group-hover/note:text-stone-600 transition-colors">
                        {item.note}
                      </span>
                    </div>
                  ) : (
                    <button
                      ...
                      className="flex items-center gap-1 text-2xs text-stone-300 hover:text-blue-500 transition-colors mb-2"
                    >
