    ...
    <div className="flex flex-col h-full">
      ...
      {checklists.length > 0 && (
        <div className="mb-3 flex flex-wrap gap-2 flex-shrink-0">
          {stats.overdue > 0 && (
            <div className="flex-1 min-w-0 rounded-xl bg-red-50 border border-red-200 px-2 py-1.5 text-center">
          ...
          <div className="flex-1 min-w-0 rounded-xl bg-amber-50 border border-amber-100 px-2 py-1.5 text-center">
            <p className="text-base font-bold text-amber-600">{stats.todo}</p>
            <p className="text-3xs text-amber-500 font-medium">📋 To-Do</p>
          </div>
          <div className="flex-1 min-w-0 rounded-xl bg-emerald-50 border border-emerald-100 px-2 py-1.5 text-center">
              ...
              <div
                ...
                className={`group flex items-center gap-2 rounded-xl px-3 py-2.5 cursor-pointer transition-all ${
                  isSelected
                    ...
                    : 'hover:bg-white/70 border border-transparent hover:border-border'
                }`}
                onClick={() => onSelectChecklist(cl.id)}
              >
                <CheckSquare className={`h-3.5 w-3.5 flex-shrink-0 ${isSelected ? 'text-primary' : 'text-stone-400'}`} />
                <div className="flex-1 min-w-0">
                  <p className={`text-xs font-medium truncate ${isSelected ? 'text-primary' : 'text-stone-700'}`}>
                    {cl.title}
                  </p>
                  <p className="text-2xs text-stone-400">{done}/{total} done</p>
                </div>
                <button
